"""
from backend.core import run_llm
import streamlit as st

st.header('LangChain - Documentation Helper Bot')

prompt = st.text_input("Prompt", placeholder="Enter your prompt here")

if (
        "chat_answer_history" not in st.session_state
        and "user_prompt_history" not in st.session_state
        and "chat_history" not in st.session_state
):
    st.session_state["chat_answer_history"] = []
    st.session_state["user_prompt_history"] = []
    st.session_state["chat_history"] = []


def create_sources_string(source_urls: set[str]) -> str:
    if not source_urls:
        return ""
    sources_list = list(source_urls)
    sources_list.sort()
    sources_string = "Sources:\n"
    for i, source in enumerate(sources_list):
        sources_string += f"{i + 1}. {source}\n"
    return sources_string


if prompt:
    with st.spinner("Generating response..."):
        generated_response = run_llm(
            query=prompt, chat_history=st.session_state["chat_history"])

        sources = set([doc.metadata["source"] for doc in generated_response["source_documents"]])
        formatted_response = (
            f"{generated_response['result']}\n\n{create_sources_string(sources)}"
        )

        st.session_state["user_prompt_history"].append(prompt)
        st.session_state["chat_answers_history"].append(formatted_response)
        st.session_state["chat_history"].append(("human", prompt))
        st.session_state["chat_history"].append("ai", generated_response["result"])

    if st.session_state["chat_answer_history"]:
        for generated_response, user_query in zip(st.session_state["chat_answer_history"],
                                                  st.session_state["user_prompt_history"]):
            message(user_query, is_user=True)
            message(generated_response)

"""

import streamlit as st
from backend.core import run_llm


# Estilos personalizados con CSS
def add_custom_css():
    st.markdown("""
        <style>
        .chat-box {
            background-color: #f0f2f6;
            border-radius: 10px;
            padding: 10px;
            margin-bottom: 10px;
            font-family: Arial, sans-serif;
            color: #333;
            
        }
        .user-box {
            background-color: #4CAF50;
            color: white;
            border-radius: 10px;
            padding: 10px;
            margin-bottom: 10px;
            text-align: right;
        }
        .bot-box {
            background-color: #2196F3;
            border-radius: 10px;
            padding: 10px;
            margin-bottom: 10px;
            text-align: left;
        }
        .stButton > button {
            background-color: #4CAF50;
            color: white;
            border-radius: 10px;
            font-size: 16px;
            padding: 8px 16px;
        }
        </style>
    """, unsafe_allow_html=True)


add_custom_css()

st.markdown("<h1 style='text-align: center; color: #4B7BEC;'>LangChain - Documentation Helper Bot</h1>",
            unsafe_allow_html=True)

prompt = st.text_input("Prompt", placeholder="Enter your prompt here")

# Inicializar el estado de la sesión para mantener el historial
if (
        "chat_answer_history" not in st.session_state
        and "user_prompt_history" not in st.session_state
        and "chat_history" not in st.session_state
):
    st.session_state["chat_answer_history"] = []
    st.session_state["user_prompt_history"] = []
    st.session_state["chat_history"] = []


# Función para crear string de fuentes
def create_sources_string(source_urls: set[str]) -> str:
    if not source_urls:
        return ""
    sources_list = list(source_urls)
    sources_list.sort()
    sources_string = "Sources:\n"
    for i, source in enumerate(sources_list):
        sources_string += f"{i + 1}. {source}\n"
    return sources_string


# Procesar el prompt
if prompt:
    with st.spinner("Generating response..."):
        generated_response = run_llm(
            query=prompt, chat_history=st.session_state["chat_history"]
        )

        sources = set([doc.metadata["source"] for doc in generated_response["source"]])
        formatted_response = (
            f"{generated_response['result']}\n\n{create_sources_string(sources)}"
        )

        # Guardar el historial de chat
        st.session_state["user_prompt_history"].append(prompt)
        st.session_state["chat_answer_history"].append(formatted_response)
        st.session_state["chat_history"].append(("human", prompt))
        st.session_state["chat_history"].append(("ai", generated_response["result"]))

# Mostrar el historial de mensajes
if st.session_state["chat_answer_history"]:
    for user_query, generated_response in zip(st.session_state["user_prompt_history"],
                                              st.session_state["chat_answer_history"]):
        st.markdown(f"<div class='user-box'>{user_query}</div>", unsafe_allow_html=True)
        st.markdown(f"<div class='bot-box'>{generated_response}</div>", unsafe_allow_html=True)
